...
public class CategoryModel {
	...
	public Integer nextPK() throws DatabaseException {
		...
		int pk = 0;
//Here we are Connecting with the database so we can an access the user datas
		try {
...
//connection building
		try {
			...
			ResultSet rs = pstmt.executeQuery();
			while (rs.next()) {
				//Filling the attributes of Category bean class
	...
	public long add(CategoryBean bean) throws ApplicationException, DuplicateRecordException {
		...
		//Data already exist then show "Data Exist"
		if (existbean != null) {
	...
	public List search(CategoryBean bean, int pageNo, int pageSize) throws ApplicationException {
		...
		// if page size is greater than zero then apply pagination(Divide the pages into discrete pages)
		if (pageSize > 0) {
