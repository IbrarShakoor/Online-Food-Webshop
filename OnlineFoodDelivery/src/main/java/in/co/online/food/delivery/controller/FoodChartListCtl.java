...
/**
 * Servlet implementation class FoodChartListCtl
 */
@WebServlet(name = "FoodChartListCtl", urlPatterns = { "/ctl/FoodChartListCtl" })
public class FoodChartListCtl extends BaseCtl {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		FoodChartBean bean = (FoodChartBean) populateBean(request);
		...
		try {
			...
			if(dsid > 0) {
				...
				FoodChartBean deletebean = new FoodChartBean();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (OP_SEARCH.equalsIgnoreCase(op) || "Next".equalsIgnoreCase(op) || "Previous".equalsIgnoreCase(op)) {
				...
				ServletUtility.redirect(OFDView.FOOD_CHART_LIST_CTL, request, response);
				return;
			}
			...
			list = model.search(bean, pageNo, pageSize);
			/*
			 * FoodChartBean be =(FoodChartBean) list;
	...
	protected String getView() {
		...
		return OFDView.FOOD_CHART_LIST_VIEW;
